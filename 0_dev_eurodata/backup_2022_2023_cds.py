...
# INIT
i=0
start_date = datetime.strptime("16/01/2024", "%d/%m/%Y") # stard_date is the most recent hitoric (end)

#
while(start_date>datetime.strptime("01/10/2023", "%d/%m/%Y")) :
    print(f">>>>>>>> Extract CDS : ({i}) <<<<<<<<<<<<")
    ...
    try :
        ...
        scheduler.run_cds(window_interval=30, interval_end = start_date.strftime("%d/%m/%Y"))
        ...
        # RUN LOAD
        from script.talend_subprocess import run_all_integration,run_cds_integration#, run_vente_carburant_integration
        run_cds_integration()
        ...
        i+=1
        rand_sleep = random.randint(60, 80) # second
